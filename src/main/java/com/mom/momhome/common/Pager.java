...
public class Pager {
	...
	public static String makeTag(HttpServletRequest request , int pageSize , int total) {
		...
		//REQUEST 객체가 있어야 pg key keyword 등의 정보를 가져올 수 있따
		//pagesize는 한페이지당 출력할 데이터 갯수
		//total - 전체 데이터 개수
		//페이지 숫자 - total/pagesize  12/10 celi함수 강제로 값 올려줌
		...
		int cpage; //현재페이지 정보 board/list?pg=1 pg값을 cpage로 처맇나다
		// 12/10 -> ceiling(1.2) -> 2   
		int pageTotal; //전체 페이지 개수 
		int pageGroupSize = 5; //그룹의 개수 한화면에 나타낼 페이지 개수 
...
//			setTotalCnt(Integer.parseInt(totCnt)) ; 
			...
			cpage = Integer.parseInt(page) ; //페이지 값 너준거
